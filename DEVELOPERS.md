...
Commands for adding, removing or modifying constraints of dependencies will
automatically respect the global timestamp cutoff specified in the
`pyproject.toml` of 7 days to align with our Tech-level dependency updates
policy.

Note that `uv.lock` must be reproducible from `pyproject.toml`.

Otherwise, the `just check` and `just check-lockfile` recipes will fail with an error saying that the timestamps must match. You might have modified one of `uv.lock` or `pyproject.toml` but not the other:
  - If you modified `pyproject.toml`:
      - update `uv.lock` via `uv lock` or `just update-dependencies`
  - If you did not modify `pyproject.toml` but have changes in `uv.lock`:
      1. revert the changes to `uv.lock`
      1. modify `pyproject.toml` as you require
      1. run `uv lock` to update the `uv.lock` file.

### Adding a package-specific timestamp cutoff

It is possible to specify a package-specific timestamp cutoff in addition to
the global cutoff.  This should be done in the `pyproject.toml` to ensure
reproducible installs; see the [uv
documentation](https://docs.astral.sh/uv/reference/settings/#exclude-newer-package)
for details.  If set, the package-specific cutoff will take precedence over the
global cutoff regardless of which one is more recent.
