...
@load_draft
def search(request, draft, search_id):
    ...
    searches = [
        {
            ...
            "url": draft.get_builder_search_url(s.id),
            "delete_url": draft.get_builder_delete_search_url(s.id),
            "active": s == search,
        }
...
def new_search(request, draft):
    ...
    return redirect(draft.get_builder_search_url(search.id))
...
@load_draft
@require_permission
def delete_search(request, draft, search_id):
    search = get_object_or_404(Search, version=draft, id=search_id)
    actions.delete_search(search=search)
    messages.info(
        request,
        f'Search for "{search.slug}" deleted. Any codes matching "{search.slug}" that '
