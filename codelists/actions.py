...
def convert_bnf_codelist_version_to_dmd(version, published=False):
    ...
    converted_codelist = create_old_style_codelist(
        ...
        name=f"{version.codelist.name} - dmd",
        slug=f"{version.codelist.slug}-dmd",
        coding_system_id="dmd",
        coding_system_database_alias=most_recent_database_alias("dmd"),
        methodology=f"Converted from [pseudo-BNF system codelist]({version.get_absolute_url()})",
        description=version.codelist.description,
        csv_data=dmd_csv_data,
        references=[
            {"text": ref.text, "url": ref.url}
            for ref in version.codelist.references.all()
        ],
    )
    if published:
        publish_version(version=converted_codelist.versions.first())
