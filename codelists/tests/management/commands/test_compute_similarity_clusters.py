...
class TestComputeSimilarityClustersCommand(TestCase):
    ...
    def test_command_basic_functionality(self):
        ...
        self.assertTrue(
            "No published versions found" in output or "Completed" in output
        )
    ...
    def test_command_force_recompute(self):
        ...
        self.assertTrue(
            "Force recomputing all signatures" in output
            or "No published versions found" in output
        )
    ...
    def test_command_threshold_parameter(self):
        ...
        self.assertTrue(
            "threshold 0.01" in output or "No published versions found" in output
        )

    @patch(
        "codelists.management.commands.compute_similarity_clusters.Command._get_latest_published_versions"
    )
