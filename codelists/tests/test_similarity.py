...
def test_version_similarity_matrix_no_similar_codelists(client, version_with_no_searches):
...
def test_version_similarity_matrix_with_similar_codelists(mock_find_similar, client, version_with_no_searches, latest_published_version):
    ...
    assert similarity_matrix[0][1]['similarity'] == similarity_matrix[1][0]['similarity']
...
@patch('codelists.views.version.find_similar_codelists')
def test_version_similarity_matrix_multiple_similar_codelists(mock_find_similar, client, version_with_no_searches, latest_published_version, user_version):
...
@patch('codelists.views.version.find_similar_codelists')
def test_version_similarity_matrix_error_handling(mock_find_similar, client, version_with_no_searches):
...
def test_similarity_matrix_computation_accuracy():
    ...
    codes1 = {'123', '456', '789'}
    codes2 = {'123', '456', '999'}  # 2 shared (123, 456), 4 total (123, 456, 789, 999) = 0.5
    ...
    # Test symmetry
    assert compute_exact_jaccard(codes1, codes2) == compute_exact_jaccard(codes2, codes1)
    assert compute_exact_jaccard(codes1, codes3) == compute_exact_jaccard(codes3, codes1)
