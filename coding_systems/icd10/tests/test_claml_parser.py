...
def test_parse_claml_handles_the_hierarchy():
    ...
    assert records["C00-C97"] == ICD10Code(
        code="C00-C97",
        parent="II",
        description="Malignant neoplasms",
    )
    assert records["II"] == ICD10Code(
        code="II",
        parent=None,
...
def test_parse_claml_excludes_reference_text_from_description_but_keeps_usage_pair():
    ...
    assert records["A170"].usage_pair_codes == ("G01",)
...
def test_parse_claml_returns_place_modifier_digits():
    ...
    assert place_modifiers == [
        ModifierDigit(digit_code="0", description="Home"),
        ModifierDigit(digit_code="1", description="Residential institution"),
        ModifierDigit(
            digit_code="2",
            description="School, other institution and public administrative area",
        ),
        ModifierDigit(digit_code="3", description="Sports and athletics area"),
        ModifierDigit(digit_code="4", description="Street and highway"),
        ModifierDigit(digit_code="5", description="Trade and service area"),
        ModifierDigit(digit_code="6", description="Industrial and construction area"),
        ModifierDigit(digit_code="7", description="Farm"),
