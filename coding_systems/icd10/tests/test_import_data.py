...
class TestImportData(DynamicDatabaseTestCase):
    ...
    @pytest.mark.usefixtures("setup_coding_systems")
    def test_import_data_skips_missing_usage_reference(self):
        output_2016 = self._parsed_output()
        output_2016["K250"].usage = ConceptUsage.DAGGER
        ...
        with (
            ...
            self._import_records(
                "missing reference", output_2016, self._parsed_output()
            )
        ...
        print_.assert_any_call(
            "DEBUG: Related code K259 for K250 in edition 2016 not found among imported concepts"
        )
