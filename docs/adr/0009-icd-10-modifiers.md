...
ICD-10 uses a system of modifiers to allow concepts to be further specified - e.g. an anatomical site where a disease is present, or the presence/non-presence of a common complication of a disease.
...
`ehrQL` is now the only means of accessing clinical event data in OpenSAFELY,
