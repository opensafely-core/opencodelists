...
def build_fixtures():
    ...
    # enthesopathy_of_elbow_region_plus_tennis_elbow
    ...
    save(draft=version_with_complete_searches)
    # Check that all code_objs are linked to searches
    assert not minimal_version.code_objs.filter(results__isnull=True).exists()
    return locals()
...
minimal_version = build_fixture("minimal_version")
