...
def save_codelist_for_review(page):
    ...
    expect(page.get_by_role("button", name="Save for review")).to_be_enabled()
...
def validate_codelist_exists_on_site(page, codelist_name, heading_text):
    ...
    # Note that this partially matches the name, unless exact=True.
    expect(page.get_by_role("link", name=codelist_name)).to_be_visible()
    expect(
