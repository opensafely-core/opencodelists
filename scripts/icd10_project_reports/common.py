...
from coding_systems.icd10.known_diffs.combined2016_vs_who2019_moved_codes import (
    MOVED_CODE_SETS,
)
...
def load_swapped_codes():
    ...
    for moved_code_set in MOVED_CODE_SETS:
        nhs2016 = set(moved_code_set["nhs2016"])
        who2019 = set(moved_code_set["who2019"])
        group = {
            "codes": sorted(who2019 - nhs2016),
            "description": moved_code_set["title"],
            "actual_codes": sorted(nhs2016 - who2019),
        }
        related_codes = sorted(nhs2016 & who2019)
        if related_codes:
            group["related_codes"] = related_codes
        groups.append(group)
    codes = {
    ...
    print(f"Loaded {len(codes)} unique moved codes from known_diffs\n")
    return codes, groups
...
def load_changed_code_definitions():
    changed = {
        code: {
            "2016": difference.combined_2016,
            "2019": difference.who_2019,
        }
        for code, difference in COMBINED_2016_VS_2019_DIFFERENCES.items()
        if not difference.clinically_equivalent
    }
