...
# Codelists named for diarrhoea intentionally target that symptom rather than every
# form of IBS, so only the directly equivalent diarrhoea codes should be suggested.
DIARRHOEA_IBS_FINDING = {
    "searched_codes": {"K581"},
    "equivalent_codes": {"K580", "K581"},
...
def format_code_list(codes, descriptions=None):
    descriptions = descriptions or {}
    return ", ".join(
        f"`{code} - {descriptions[code]}`" if code in descriptions else f"`{code}`"
    ...
    def format_repo_report(repo, files_by_path):
        ...
        for path, matches in sorted(files_by_path.items()):
            ...
            for group in groups:
                ...
                if not searched_codes & match_codes:
                    continue
                exception = (
                    DIARRHOEA_IBS_FINDING if "diarrhoea" in path.lower() else None
                )
                if (
                    exception
                    and exception["searched_codes"] & searched_codes & match_codes
                ):
                    equivalent_codes = exception["equivalent_codes"]
                    description = exception["description"]
                    code_descriptions = exception.get("code_descriptions", {})
                else:
                    equivalent_codes = (
                        searched_codes
                ...
                for finding in findings:
                    ...
                    lines.extend(
                        [
                            ...
                            f"{format_code_list(finding['found'], finding['code_descriptions'])}",
                            "  - Codes potentially missing from this codelist: "
                            f"{format_code_list(finding['missing'], finding['code_descriptions'])}",
        ...
        def render_report(report_sections):
            ...
            lines = [
                ...
                f"we've identified that the repository, [{repo_name}]"
