    ...
    while True:
        ...
        if response.status_code == 429:
            if response.headers.get("X-RateLimit-Reset"):
                reset_time = int(response.headers["X-RateLimit-Reset"]) / 1000
...
def check_status():
    ...
    for item in items:
        print(
            f"{datetime.datetime.fromtimestamp(item['timestamp'], tz=datetime.UTC)}: {item['event']} to {item['message']['headers']['to']}"
        )
