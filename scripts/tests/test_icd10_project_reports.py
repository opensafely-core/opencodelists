...
def test_diarrhoea_codelist_uses_specific_moved_code_exception(
    tmp_path, monkeypatch, repo_name
):
    monkeypatch.setattr(reports, "convert_markdown_to_pdf", lambda path: None)

    def github_response_for_diarrhoea(args):
        if "contents/" in " ".join(args):
            content = "code,description\nK581,IBS with predominant diarrhoea\n"
            encoded = base64.b64encode(content.encode()).decode()
            return True, json.dumps({"content": encoded, "encoding": "base64"})
        return True, json.dumps({"default_branch": "main"})

    monkeypatch.setattr(reports, "run_gh_command", github_response_for_diarrhoea)
    moved_codes, groups = common.load_swapped_codes()

    reports.generate_repo_emails(
        {
            "K581": {
                f"opensafely/{repo_name}": [
                    {
                        "path": "codelists/bristol-diarrhoea-icd10.csv",
                        "line_text": "K581,IBS with predominant diarrhoea",
                    }
                ]
            }
        },
        ...
        groups,
        {
            "K580": {("apcs_all_count", "TOTAL"): 29_440},
            "K581": {("apcs_all_count", "TOTAL"): 305_740},
        },
        {},
        project_types={f"opensafely/{repo_name}": "cohortextractor"},
        reports_dir=tmp_path,
    )

    report = (tmp_path / "cohortextractor" / f"{repo_name}.md").read_text()
    assert "**Irritable bowel syndrome with diarrhoea:**" in report
    assert (
        "`K581 - Irritable bowel syndrome with predominant diarrhoea [IBS-D]`" in report
    )
    assert "`K580 - Irritable bowel syndrome with diarrhoea`" in report
    assert "305,740 APCS events" in report
    assert "335,180 APCS events" in report
    assert "K582" not in report
    assert "K583" not in report
    assert "K588" not in report
    assert "K589" not in report
...
def test_known_diff_definitions_load():
    moved_codes, groups = common.load_swapped_codes()
    changed_definitions = common.load_changed_code_definitions()

    assert moved_codes["A925"] == "Zika virus disease"
    assert {
        "codes": ["K581", "K582", "K583", "K588"],
        "description": "Irritable bowel syndrome",
        "actual_codes": ["K580", "K589"],
        "related_codes": ["K58"],
    } in groups
    assert changed_definitions["I620"] == {
        "2016": "Subdural haemorrhage (acute)(nontraumatic)",
...
def test_project_type_overrides_are_limited_to_known_exceptions():
    assert common.load_project_type_overrides() == {
        "tpp-code-counts": "ignore",
        "comparative-booster-spring2023": "ehrql",
    }
